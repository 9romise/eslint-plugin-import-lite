...
function groupBy<T>(array: T[], grouper: (curr: T, index: number) => string | number) {
  return array.reduce<Record<string, T[]>>((acc, curr, index) => {
...
function isCJSExports(context: RuleContext, node: TSESTree.Expression) {
  if (
    ...
    return !context.sourceCode
      .getScope(node)
      .variables
      .some((variable) => variable.name === 'module')
  }
...
function getNormalizedValue(node: ImportEntry, toLowerCase?: boolean) {
...
function convertPathGroupsForRanks(pathGroups: PathGroup[]) {
  ...
  return {
    ...
    maxPosition:
      maxPosition > 10 ? 10 ** Math.ceil(Math.log10(maxPosition)) : 10,
  }
}
